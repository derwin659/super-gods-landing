...
export default function OwnerShowcasePage() {
  ...
  const collectionOptions=useMemo(()=>[...new Set(items.map((item)=>item.collectionName).filter(Boolean))].sort((a,b)=>a.localeCompare(b)),[items]);
  ...
  async function openReports(item){
    setReportPanel(item);setReportRows([]);setReportLoading(true);setError('');
    try{setReportRows(await getOwnerShowcaseReports(item.id));}
    catch(e){setError(e.message||'No se pudieron cargar los reportes.');}
    finally{setReportLoading(false);}
  }
  async function resolveReport(report,decision){
    const note=(reportNotes[report.reportId]||'').trim();
    if(!note){setError('Escribe una nota de auditoría antes de resolver el reporte.');return;}
    if(decision==='CONTENT_HIDDEN'&&!window.confirm('¿Retirar esta publicación de la vitrina? Dejará de ser visible inmediatamente.'))return;
    setWorking(report.reportId);setError('');
    try{
      await resolveOwnerShowcaseReport(report.reportId,decision,note);
      setSuccess(decision==='CONTENT_HIDDEN'?'Publicación retirada y reporte resuelto.':'Reporte revisado correctamente.');
      setReportRows(await getOwnerShowcaseReports(reportPanel.id));
      if(decision==='CONTENT_HIDDEN'){setReportPanel(null);await load();}
    }catch(e){setError(e.message||'No se pudo resolver el reporte.');}
    finally{setWorking(0);}
  }
  ...
  return <div className="space-y-6">
    ...
    {showCreator&&<form onSubmit={submitCatalog} className="rounded-[30px] border border-amber-200 bg-[linear-gradient(145deg,#fff,#fffbeb)] p-6 shadow-lg">
      ...
      {previewUrl&&<section className="mt-5 overflow-hidden rounded-[26px] border border-amber-200 bg-white shadow-sm"><div className="grid lg:grid-cols-[minmax(0,1.15fr)_minmax(280px,.85fr)]"><div className="relative min-h-64 bg-neutral-950">{form.mediaType==='VIDEO'?<video src={previewUrl} controls muted playsInline onLoadedMetadata={(event)=>setForm((current)=>({...current,durationSeconds:String(Math.max(1,Math.ceil(event.currentTarget.duration||1)))}))} className="h-full max-h-[520px] w-full object-contain"/>:<img src={previewUrl} alt="Vista previa" className="h-full max-h-[520px] w-full object-contain"/>}<span className="absolute left-4 top-4 rounded-full bg-black/75 px-3 py-1 text-xs font-black text-white">VISTA PREVIA · {form.mediaType==='VIDEO'?'VIDEO':'FOTO'}</span></div><div className="flex flex-col justify-center p-6"><div className="text-xs font-black uppercase tracking-[.18em] text-amber-700">Así lo verá el cliente</div><h3 className="mt-3 text-2xl font-black text-neutral-950">{form.title.trim()||'Título de la publicación'}</h3><p className="mt-2 font-bold text-neutral-500">{[form.collectionName.trim(),form.category.trim()].filter(Boolean).join(' · ')||'Inspiración del negocio'}</p><p className="mt-4 text-sm leading-6 text-neutral-600">{form.description.trim()||'Agrega una descripción para contar qué hace especial este trabajo.'}</p>{form.mediaType==='VIDEO'&&<div className="mt-4 rounded-2xl bg-neutral-100 px-4 py-3 text-sm font-black text-neutral-700">Duración detectada: {form.durationSeconds||'calculando…'} s</div>}<button type="button" onClick={()=>setForm((current)=>({...current,file:null,durationSeconds:''}))} className="mt-5 rounded-2xl border border-neutral-200 px-4 py-3 text-sm font-black text-neutral-600">Cambiar archivo</button></div></div></section>}
    ...
    {loading?<div className="rounded-3xl bg-white p-10 text-center font-black">Cargando trabajos...</div>:visibleItems.length===0?<div className="rounded-3xl border border-dashed border-neutral-300 bg-white p-12 text-center"><Image className="mx-auto text-neutral-300" size={55}/><h2 className="mt-4 text-xl font-black">{items.length===0?'No hay trabajos en este estado':'Ninguna publicación coincide con los filtros'}</h2><p className="mx-auto mt-2 max-w-xl text-sm font-semibold text-neutral-500">{items.length===0?'Cuando exista contenido aparecerá en esta sección.':'Prueba limpiando uno o más filtros para ampliar los resultados.'}</p>{items.length>0&&<button type="button" onClick={()=>setListFilters(initialListFilters)} className="mt-5 rounded-2xl bg-neutral-950 px-5 py-3 text-sm font-black text-white">Limpiar filtros</button>}</div>:<div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{visibleItems.map((item)=>{const catalog=item.originType==='TENANT_CATALOG';return <article key={item.id} className="overflow-hidden rounded-[26px] border border-neutral-200 bg-white shadow-sm"><div className="relative aspect-[4/3] bg-neutral-100">{item.mediaType==='VIDEO'?<video src={item.imageUrl} poster={item.thumbnailUrl} controls preload="metadata" className="h-full w-full object-cover"/>:<img src={item.imageUrl} alt={item.title} className="h-full w-full object-cover"/>}<span className="absolute left-3 top-3 rounded-full bg-black/75 px-3 py-1 text-xs font-black text-white">{item.mediaType==='VIDEO'?'VIDEO':'FOTO'}</span>{item.featured&&<span className="absolute bottom-3 left-3 flex items-center gap-1 rounded-full bg-amber-300 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-neutral-950 shadow"><Star size={13} fill="currentColor"/>Destacado · prioridad {item.sortOrder||1}</span>}<span className={`absolute right-3 top-3 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider ${catalog?'bg-amber-300 text-neutral-950':'bg-white/90 text-neutral-700'}`}>{catalog?'Inspiración del negocio':'Trabajo profesional'}</span></div><div className="p-5"><div className="text-xs font-black uppercase tracking-wider text-amber-700">{catalog?(item.visibilityScope==='ALL_BRANCHES'?'Todas las sedes':item.visibilityScope==='SELECTED_BRANCHES'?`${item.branchIds?.length||0} sedes`:item.branchName):item.branchName}</div><h2 className="mt-2 text-xl font-black">{item.title}</h2><p className="mt-1 text-sm font-bold text-neutral-500">{catalog?([item.collectionName,item.category].filter(Boolean).join(' · ')||'Catálogo del negocio'):[item.professionalName,item.serviceName].filter(Boolean).join(' · ')}</p>{catalog&&item.publishedAt&&new Date(item.publishedAt)>new Date()&&<p className="mt-3 rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-xs font-black text-sky-800">Programada para {new Date(item.publishedAt).toLocaleString('es-PE',{dateStyle:'medium',timeStyle:'short'})}</p>}{item.description&&<p className="mt-3 text-sm text-neutral-600">{item.description}</p>}{Number(metrics[String(item.id)]?.openReports||0)>0&&<button type="button" onClick={()=>openReports(item)} className="mt-3 flex w-full items-center justify-between gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-left text-xs font-black text-rose-700 transition hover:bg-rose-100"><span className="flex items-center gap-2"><AlertTriangle size={16}/>{Number(metrics[String(item.id)]?.openReports||0)} reporte(s) por revisar</span><span>Revisar →</span></button>}{(()=>{const m=metrics[String(item.id)]||{};return <div className="mt-4 grid grid-cols-2 gap-2 rounded-2xl sm:grid-cols-5 bg-neutral-50 p-3"><div className="text-center"><Eye className="mx-auto text-neutral-400" size={16}/><div className="mt-1 text-sm font-black">{Number(m.views||0)}</div><div className="text-[9px] font-bold uppercase text-neutral-400">Vistas</div></div><div className="text-center"><PlayCircle className="mx-auto text-neutral-400" size={16}/><div className="mt-1 text-sm font-black">{Number(m.videoStarts||0)} / {Number(m.videoCompletes||0)}</div><div className="text-[9px] font-bold uppercase text-neutral-400">Inicio / fin</div></div><div className="text-center"><MousePointerClick className="mx-auto text-neutral-400" size={16}/><div className="mt-1 text-sm font-black">{Number(m.reserveClicks||0)}</div><div className="text-[9px] font-bold uppercase text-neutral-400">Reservar</div></div><div className="text-center"><Heart className="mx-auto text-rose-400" size={16}/><div className="mt-1 text-sm font-black">{Number(m.favorites||0)}</div><div className="text-[9px] font-bold uppercase text-neutral-400">Guardados</div></div><div className="text-center"><Share2 className="mx-auto text-indigo-400" size={16}/><div className="mt-1 text-sm font-black">{Number(m.shares||0)}</div><div className="text-[9px] font-bold uppercase text-neutral-400">Compartidos</div></div></div>})()}{item.rejectionReason&&<p className="mt-3 rounded-xl bg-red-50 p-3 text-sm font-bold text-red-700">{item.rejectionReason}</p>}{status==='PENDING_APPROVAL'&&<div className="mt-5 grid grid-cols-2 gap-3"><button disabled={working===item.id} onClick={()=>openReject(item)} className="flex items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 font-black text-red-700"><X size={18}/>Rechazar</button><button disabled={working===item.id} onClick={()=>moderate(item,'PUBLISHED')} className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-4 py-3 font-black text-white"><Check size={18}/>Publicar</button></div>}{status==='PUBLISHED'&&<div className="mt-5 space-y-3"><button disabled={working===item.id} onClick={()=>manageFeatured(item,'toggle')} className={`flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3 font-black ${item.featured?'border border-amber-300 bg-amber-50 text-amber-800':'bg-amber-400 text-neutral-950'}`}><Star size={18} fill={item.featured?'currentColor':'none'}/>{item.featured?'Quitar destacado':'Destacar publicación'}</button>{item.featured&&<div className="grid grid-cols-2 gap-3"><button disabled={working===item.id} onClick={()=>manageFeatured(item,'UP')} className="flex items-center justify-center gap-2 rounded-2xl border border-neutral-200 px-3 py-2.5 text-sm font-black text-neutral-700"><ArrowUp size={17}/>Subir</button><button disabled={working===item.id} onClick={()=>manageFeatured(item,'DOWN')} className="flex items-center justify-center gap-2 rounded-2xl border border-neutral-200 px-3 py-2.5 text-sm font-black text-neutral-700"><ArrowDown size={17}/>Bajar</button></div>}<button disabled={working===item.id} onClick={()=>manage(item,'archive')} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-neutral-950 px-4 py-3 font-black text-white"><Archive size={18}/>Pausar publicación</button></div>}{status==='ARCHIVED'&&<div className="mt-5 grid grid-cols-2 gap-3"><button disabled={working===item.id} onClick={()=>manage(item,'delete')} className="flex items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 font-black text-red-700"><Trash2 size={18}/>Eliminar</button><button disabled={working===item.id} onClick={()=>manage(item,'publish')} className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-4 py-3 font-black text-white"><RotateCcw size={18}/>Republicar</button></div>}</div></article>})}</div>}
    {reportPanel&&<div className="fixed inset-0 z-[85] flex items-end justify-center bg-neutral-950/75 p-0 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true"><section className="max-h-[94vh] w-full max-w-4xl overflow-y-auto rounded-t-[30px] bg-white shadow-2xl sm:rounded-[30px]"><header className="sticky top-0 z-10 flex items-start justify-between gap-4 bg-rose-900 p-6 text-white"><div><div className="text-[10px] font-black uppercase tracking-[.2em] text-rose-200">Seguridad de Vitrina</div><h2 className="mt-2 text-2xl font-black">Reportes de {reportPanel.title}</h2><p className="mt-2 text-sm font-semibold text-white/70">Documenta cada decisión. No mostramos la identidad del visitante.</p></div><button type="button" onClick={()=>setReportPanel(null)} disabled={Boolean(working)} className="grid h-10 w-10 place-items-center rounded-full bg-white/10"><X size={20}/></button></header><div className="space-y-4 p-6">{error&&<div className="rounded-2xl bg-red-50 p-4 text-sm font-bold text-red-700">{error}</div>}{reportLoading?<div className="p-10 text-center font-black">Cargando reportes…</div>:reportRows.length===0?<div className="rounded-2xl bg-neutral-50 p-8 text-center font-bold text-neutral-500">No hay reportes.</div>:reportRows.map((report)=><article key={report.reportId} className={'rounded-2xl border p-5 '+(report.status==='OPEN'?'border-rose-200 bg-rose-50/40':'border-neutral-200 bg-neutral-50 opacity-70')}><div className="flex justify-between gap-3"><div><div className="text-xs font-black uppercase text-rose-700">{({INAPPROPRIATE:'Contenido inapropiado',PERSONAL_DATA:'Datos personales',NO_CONSENT:'Falta autorización',MISLEADING:'Contenido engañoso',SPAM:'Spam',OTHER:'Otro'})[report.reason]||report.reason}</div><div className="mt-1 text-xs font-bold text-neutral-400">{new Date(report.createdAt).toLocaleString('es-PE')}</div></div><span className={'h-fit rounded-full px-3 py-1 text-[10px] font-black uppercase '+(report.status==='OPEN'?'bg-rose-600 text-white':'bg-neutral-200 text-neutral-600')}>{report.status==='OPEN'?'Pendiente':'Resuelto'}</span></div>{report.details&&<p className="mt-4 rounded-xl bg-white p-3 text-sm font-semibold">{report.details}</p>}{report.status==='OPEN'?<><textarea maxLength={300} rows={3} value={reportNotes[report.reportId]||''} onChange={(e)=>setReportNotes({...reportNotes,[report.reportId]:e.target.value})} placeholder="Nota obligatoria de auditoría" className="mt-4 w-full resize-none rounded-xl border border-neutral-200 bg-white p-3 text-sm font-semibold"/><div className="mt-3 grid gap-3 sm:grid-cols-2"><button type="button" disabled={working===report.reportId} onClick={()=>resolveReport(report,'DISMISSED')} className="rounded-xl border border-neutral-300 bg-white px-4 py-3 text-sm font-black">Descartar reporte</button><button type="button" disabled={working===report.reportId} onClick={()=>resolveReport(report,'CONTENT_HIDDEN')} className="rounded-xl bg-rose-600 px-4 py-3 text-sm font-black text-white">Retirar publicación</button></div></>:<p className="mt-4 text-sm font-bold text-neutral-500">Nota: {report.resolutionNote||'Sin nota'}</p>}</article>)}</div></section></div>}
