...
function FundMovementReportSection({ report }) {
  ...
  return (
    <section className="overflow-hidden rounded-[26px] border border-amber-400/20 bg-[linear-gradient(135deg,#0F172A_0%,#211A2C_100%)] p-4 text-white shadow-[0_18px_46px_rgba(15,23,42,0.14)] sm:rounded-[30px] sm:p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-300">Ingresos, retiros y variación del fondo sin mezclarlos con la caja diaria.</p>
        </div>
      ...
      {methods.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {methods.map((method) => (
            <span key={method.paymentMethod} className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-2 text-xs font-bold text-slate-200">
              {methodLabel(method.paymentMethod)} · neto {formatMoney(method.netMovement)}
            </span>
          ))}
        </div>
      )}

      <div className="mt-5 max-h-[400px] overflow-auto rounded-2xl border border-white/10">
        <table className="w-full min-w-[1050px] text-left text-sm">
          <thead className="sticky top-0 bg-slate-950 text-[10px] font-black uppercase tracking-wider text-slate-400">
            <tr><th className="px-4 py-3">Fecha</th><th className="px-4 py-3">Tipo</th><th className="px-4 py-3">Concepto</th><th className="px-4 py-3">Método</th><th className="px-4 py-3">Sede</th><th className="px-4 py-3">Responsable / caja</th><th className="px-4 py-3 text-right">Monto</th></tr>
          </thead>
          <tbody className="divide-y divide-white/10">
            {items.map((item) => {
              const income = n(item.signedAmount) >= 0;
              return (
                <tr key={item.id} className="align-top transition hover:bg-white/[0.04]">
                  <td className="px-4 py-3 font-bold text-slate-300">{String(item.date || '').replace('T', ' ').slice(0, 16)}</td>
                  <td className={`px-4 py-3 font-black ${income ? 'text-emerald-300' : 'text-red-300'}`}>{typeLabel(item.type)}</td>
                  <td className="px-4 py-3"><p className="font-black">{item.concept || 'Movimiento de fondo'}</p>{item.note ? <p className="mt-1 max-w-xs whitespace-normal text-xs font-semibold text-slate-400">{item.note}</p> : null}</td>
                  <td className="px-4 py-3 font-bold text-blue-300">{methodLabel(item.paymentMethod)}</td>
                  <td className="px-4 py-3 font-bold">{item.branchName || 'Sede'}</td>
                  ...
                  <td className={`px-4 py-3 text-right font-black ${income ? 'text-emerald-300' : 'text-red-300'}`}>{income ? '+' : '-'}{formatMoney(Math.abs(n(item.amount)))}</td>
                </tr>
              );
            })}
            {items.length === 0 && <tr><td colSpan="7" className="px-4 py-9 text-center font-bold text-slate-400">No hubo movimientos del fondo en este rango.</td></tr>}
          </tbody>
        </table>
      </div>
    </section>
...
export default function OwnerReportsPage() {
  ...
  const [fundMovementReport, setFundMovementReport] = useState(null);
  ...
  async function loadReports() {
    ...
    try {
      ...
      setPeriodComparison(periodComparisonData);
      setFundMovementReport(fundMovementData);
    } catch (error) {
...
<div className="flex flex-wrap gap-2"><button type="button" onClick={() => exportReports("excel")} disabled={loading || Boolean(exporting)} className="rounded-2xl border border-emerald-300/40 bg-emerald-400/15 px-4 py-3 text-sm font-black text-emerald-200 disabled:opacity-50">{exporting === "excel" ? "Generando..." : "Excel"}</button><button type="button" onClick={() => exportReports("pdf")} disabled={loading || Boolean(exporting)} className="rounded-2xl border border-red-300/40 bg-red-400/15 px-4 py-3 text-sm font-black text-red-100 disabled:opacity-50">{exporting === "pdf" ? "Generando..." : "PDF"}</button><button type="button" onClick={loadReports} disabled={loading} className="rounded-2xl bg-amber-400 px-5 py-3 text-sm font-black text-neutral-950 disabled:opacity-60">{loading ? "Actualizando..." : "Actualizar"}</button></div>       </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <FundMovementReportSection report={fundMovementReport} />
...
<section className="rounded-[24px] border border-neutral-200 bg-white p-4 shadow-[0_14px_38px_rgba(15,23,42,0.05)] sm:rounded-[30px] sm:p-5">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between"><div><h3 className="text-xl font-black text-neutral-950">Pagos profesionales</h3><p className="mt-1 text-sm text-neutral-500">Aquí aparecen adelantos y liquidaciones del equipo. Los gastos operativos se muestran en “Gastos detallados”.</p></div><div className="flex flex-col gap-2 sm:flex-row"><PremiumReportPicker label="Profesional" value={paymentBarberId} onChange={setPaymentBarberId} options={[{ value: "", label: "Todos los profesionales", icon: "✦" }, ...Array.from(new Map([...(professionalPaymentReport?.professionals || []).map((professional) => [String(professional.barberUserId), { value: professional.barberUserId, label: professional.barberName, icon: "♙" }]), ...barbers.map((barber) => [String(barber.barberId), { value: barber.barberId, label: barber.barberName, icon: "♙" }])]).values())]} /><PremiumReportPicker label="Estado" value={paymentStatus} onChange={setPaymentStatus} options={[{ value: "", label: "Todos los estados" }, { value: "PENDING", label: "Pendiente" }, { value: "PARTIAL", label: "Pago parcial" }, { value: "PAID", label: "Pagado" }, { value: "CANCELLED", label: "Cancelado" }]} /></div></div>
            <div className="mt-4 grid gap-3 sm:grid-cols-3 xl:grid-cols-6"><StatCard label="Sueldos" value={formatMoney(professionalPaymentReport?.totalSalary)} /><StatCard label="Comisiones" value={formatMoney(professionalPaymentReport?.totalCommissions)} /><StatCard label="Propinas" value={formatMoney(professionalPaymentReport?.totalTips)} /><StatCard label="Adelantos descontados" value={formatMoney(professionalPaymentReport?.totalAdvances)} tone="red" /><StatCard label="Pagado" value={formatMoney(professionalPaymentReport?.totalPaid)} tone="green" /><StatCard label="Pendiente" value={formatMoney(professionalPaymentReport?.totalPending)} tone="amber" /></div>
            <div className="mt-5 rounded-2xl border border-red-200 bg-red-50 p-4">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-red-700">Adelantos entregados en el rango</p><p className="mt-1 text-sm font-bold text-red-900">Movimientos reales de caja según las fechas y el profesional seleccionado.</p></div><p className="text-2xl font-black text-red-800">{formatMoney(professionalPaymentReport?.totalAdvancesDelivered)}</p></div>
              <div className="mt-3 max-h-[300px] overflow-auto rounded-xl border border-red-100 bg-white"><table className="w-full min-w-[760px] text-left text-sm"><thead className="sticky top-0 bg-red-50 text-xs font-black uppercase text-red-700"><tr><th className="px-4 py-3">Profesional</th><th className="px-4 py-3">Fecha</th><th className="px-4 py-3">Sede</th><th className="px-4 py-3">Concepto</th><th className="px-4 py-3">Método</th><th className="px-4 py-3 text-right">Monto</th></tr></thead><tbody className="divide-y divide-neutral-100">{(professionalPaymentReport?.advanceItems || []).map((item) => <tr key={item.movementId}><td className="px-4 py-3 font-black">{item.barberName}</td><td className="px-4 py-3 font-bold">{item.date}</td><td className="px-4 py-3">{item.branchName || "-"}</td><td className="px-4 py-3">{item.concept || item.note || "Adelanto"}</td><td className="px-4 py-3">{item.paymentMethod || item.fundingSource || "-"}</td><td className="px-4 py-3 text-right font-black text-red-700">{formatMoney(item.amount)}</td></tr>)}{(professionalPaymentReport?.advanceItems || []).length === 0 && <tr><td colSpan="6" className="px-4 py-8 text-center font-bold text-neutral-400">No hay adelantos entregados para estos filtros.</td></tr>}</tbody></table></div>
            </div>            <div className="mt-5 max-h-[420px] overflow-auto rounded-2xl border border-neutral-200"><table className="w-full min-w-[960px] text-left text-sm"><thead className="sticky top-0 bg-neutral-50 text-xs font-black uppercase text-neutral-400"><tr><th className="px-4 py-3">Profesional</th><th className="px-4 py-3">Periodo</th><th className="px-4 py-3">Estado</th><th className="px-4 py-3 text-right">Sueldo</th><th className="px-4 py-3 text-right">Comisión</th><th className="px-4 py-3 text-right">Propinas</th><th className="px-4 py-3 text-right">Adelantos</th><th className="px-4 py-3 text-right">Pagado</th><th className="px-4 py-3 text-right">Saldo</th></tr></thead><tbody className="divide-y divide-neutral-100">{(professionalPaymentReport?.items || []).map((item) => <tr key={item.paymentId}><td className="px-4 py-3"><p className="font-black">{item.barberName}</p><p className="text-xs text-neutral-400">{item.branchName}</p></td><td className="px-4 py-3 font-bold">{item.periodFrom} → {item.periodTo}</td><td className="px-4 py-3"><span className={`rounded-full px-2.5 py-1 text-xs font-black ${item.status === "PAID" ? "bg-emerald-100 text-emerald-800" : item.status === "CANCELLED" ? "bg-neutral-200 text-neutral-600" : "bg-amber-100 text-amber-800"}`}>{({ PAID: "Pagado", PENDING: "Pendiente", PARTIAL: "Parcial", CANCELLED: "Cancelado" })[item.status] || item.status}</span></td><td className="px-4 py-3 text-right font-bold">{formatMoney(item.salaryAmount)}</td><td className="px-4 py-3 text-right font-bold">{formatMoney(item.commissionAmount)}</td><td className="px-4 py-3 text-right font-bold">{formatMoney(item.tipsAmount)}</td><td className="px-4 py-3 text-right font-bold text-red-700">{formatMoney(item.advancesApplied)}</td><td className="px-4 py-3 text-right font-black text-emerald-700">{formatMoney(item.amountPaid)}</td><td className="px-4 py-3 text-right font-black text-amber-700">{formatMoney(item.remainingAmount)}</td></tr>)}{(professionalPaymentReport?.items || []).length === 0 && <tr><td colSpan="9" className="px-4 py-10 text-center font-bold text-neutral-400">No hay liquidaciones para estos filtros.</td></tr>}</tbody></table></div>
          </section>
          <section className="rounded-[24px] border border-neutral-200 bg-white p-4 shadow-[0_14px_38px_rgba(15,23,42,0.05)] sm:rounded-[30px] sm:p-5">
